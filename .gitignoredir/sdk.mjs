...
// (c) SAAAM LLC | SAAAM INTELLIGENCE | saaam-intelligence.com
...
function createSharedQuery({
  ...
  if (!pathToNexusCodeExecutable) {
    throw new Error("pathToNexusCodeExecutable is required");
  }
  const allMcpServers = {};
...
function query({
  ...
  let pathToNexusCodeExecutable = options?.pathToNexusCodeExecutable;
  if (!pathToNexusCodeExecutable) {
    const filename = fileURLToPath(import.meta.url);
    const dirname = join(filename, "..");
    pathToNexusCodeExecutable = join(dirname, "cli.js");
  }
  return createSharedQuery({
    prompt,
    options: { ...options, pathToNexusCodeExecutable }
  });
}
export {
  tool,
